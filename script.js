...
/*function getIconHTML(iconFile, altText, fallbackIcon = '🍽️') {
    return `
        <img src="icon/${iconFile}" 
             alt="${altText}"
             class="icon-image"
             onerror="this.onerror=null; this.style.display='none'; this.parentNode.innerHTML='${fallbackIcon}';">
    `;
}*/
...
function getDefaultData() {
    return {
        ...
        categories: [
            { id: 1, name_ar: "المشويات", name_ckb: "برژاوەکان", icon: "mashwyat.png" },  // ← تغيير هنا
            { id: 2, name_ar: "البيتزا", name_ckb: "پیتزاكان", icon: "bigpizza.png" },    // ← تغيير هنا
            { id: 3, name_ar: "المشروبات", name_ckb: "خواردنەوه‌كان", icon: "drinks.png" }  // ← تغيير هنا
        ],
        menuItems: [
            {
                id: 1,
            ...
            },
            {
                ...
                price: 3000,
                icon: "chickenkebab.png"  // ← تغيير هنا
            },
            {
                id: 3,
                categoryId: 1,
            ...
            },
            {
                ...
                price: 5000,
                icon: "middlepizza.png"  // ← تغيير هنا
            },
            {
                id: 8,
                categoryId: 2,
            ...
            },
            {
                ...
                price: 3500,
                icon: "margrittaPizza.png"  // ← تغيير هنا
            },
            {
                id: 10,
                categoryId: 3,
            ...
            },
            {
                ...
                price: 1500,
                icon: "orangejuice.png"  // ← تغيير هنا
            },
            {
                id: 14,
                categoryId: 3,
                ...
                price: 2000,
                icon: "pomegranatejuice.png"  // ← تغيير هنا
            },
            {
                id: 15,
                categoryId: 3,
...
function renderCategories() {
    ...
    const allCategories = [
        { id: 'all', name_ar: 'جميع الأصناف', name_ckb: 'هەموو خواردنەکان', icon: 'all.png' },  // ← يمكنك إضافة أيقونة "الكل"
        ...categories
    ];
    ...
    allCategories.forEach(category => {
        ...
        // استخدام getIconHTML للحصول على أيقونة مع fallback
        const iconHTML = category.id === 'all' 
            ? '<div class="all-icon">🍽️</div>'  // رمز تعبيري للفئة "الكل"
            : getIconHTML(category.icon, category[`name_${state.currentLanguage}`], '🍽️');
    ...
    filteredItems.forEach(item => {
        ...
        menuItemElement.innerHTML = `
            <div class="menu-item-header">
                <div class="item-icon">${getIconHTML(item.icon, item[`name_${state.currentLanguage}`], '🍽️')}</div>
                <div class="item-info">
...
function addToOrder(itemId) {
    ...
    if (existingOrderItem) {
        ...
        state.order.push({
            ...
            icon: menuItem.icon,  // تخزين اسم الملف فقط
            quantity: 1
        });
    }
            ...
            orderItemElement.innerHTML = `
                <div class="order-item-info">
                    <div class="order-item-header">
                        <div class="order-item-icon">${getIconHTML(orderItem.icon, orderItem[`name_${state.currentLanguage}`], '🍽️')}</div>
                        <h4>${orderItem[`name_${state.currentLanguage}`]}</h4>
                    </div>
